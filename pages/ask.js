...
function ask() {
    const [titre,setTitre] = useState("Why do we use it?")
    const [category,setCategory] = useState("web")
    const [CodeError,setCodeError] = useState("It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using 'Content here, content here', making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for 'lorem ipsum' will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on purpose (injected humour and the like).")
    const  selectHandler = (e)  =>{
   
        setCategory(e.target.value)
   }
    const [CoverImage,SetCoverImage] = useState("https://www.pierre-giraud.com/wp-content/uploads/2019/05/javascript-code-element-script-html.png")
...
<div className="mb-4">
      <input onChange={(e) => setTitre(e.target.value)} required className="appearance-none block w-full py-3 px-4 leading-tight text-gray-700 bg-gray-50 focus:bg-white border border-gray-200 focus:border-gray-500 rounded focus:outline-none" type="text" placeholder="veuillez entrer le titre"/>
    </div>
        <textarea  onChange={(e) => setCodeError(e.target.value)} placeholder="copier votre erreur ici" className="h-72 md:mt-4 mt-1 block w-full py-3 px-4 leading-tight text-gray-700 bg-gray-50 focus:bg-white border border-gray-200 focus:border-gray-500 rounded focus:outline-none">          
        </textarea>
...
<input className="hidden" type="file" ref={filePickRef} onChange={addImageToPost} />
        <img className=" h-80 w-full" src={CoverImage} />
     ...
     <option value="web">web</option>
       ...
       <div className="flex flex-col h-4/5  bg-white shadow-md  rounded-sm  mt-6 pb-5 px-2 ">
        ...
        <p className="text-gray-700 text-lg font-bold italic">{titre}</p>
        <p className="text-gray-700">erreur recontré :</p>
        <div className="bg-gray-100  h-48 overflow-scroll ">
          <p className="w-full break-words">{CodeError}</p>

        </div>
        <p className="text-gray-700">code associé :</p>
        <img className="h-80" src={CoverImage} />
   ...
   {category}
        ...
        <div className="  bg-blue-700 text-white self-center  text-center px-4 py-4  rounded-lg my-2  w-48 flex items-center justify-center mr-4 hover:bg-blue-900 cursor-pointer">
